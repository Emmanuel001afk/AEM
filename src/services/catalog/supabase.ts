...
export async function loadSupabaseCatalog():Promise<StoreApp[]>{
 ...
 return rows.map((a:any)=>{
  ...
  const latest=(channel:string)=>{
   const list=releases.filter((x:any)=>{if(x.status!=="published"||x.channel!==channel)return false;const android=(x.artifacts||[]).filter((z:any)=>z.platform==="android"&&["apk","apks","xapk","apkm"].includes(z.kind));if(!android.length)return false;return android.some((z:any)=>z.package_identity&&Number(z.version_code??x.version_code??0)>=Number(x.version_code??0));}).sort((x:any,y:any)=>{const yc=Number(y.version_code??0),xc=Number(x.version_code??0);if(yc!==xc)return yc-xc;return new Date(y.published_at||y.created_at).getTime()-new Date(x.published_at||x.created_at).getTime();});
