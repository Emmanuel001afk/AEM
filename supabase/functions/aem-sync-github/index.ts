...
async function zipApks(buf:Uint8Array){const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength);const u16=(p:number)=>dv.getUint16(p,true),u32=(p:number)=>dv.getUint32(p,true);let e=Math.min(buf.length-22,buf.length-65558);for(let p=buf.length-22;p>=e;p--){if(u32(p)===0x06054b50){const n=u16(p+10),off=u32(p+16),out:any[]=[];let q=off;for(let i=0;i<n&&q+46<=buf.length;i++){if(u32(q)!==0x02014b50)break;const method=u16(q+10),cs=u32(q+20),us=u32(q+24),nl=u16(q+28),xl=u16(q+30),cl=u16(q+32),lo=u32(q+42);const name=new TextDecoder().decode(buf.slice(q+46,q+46+nl));q+=46+nl+xl+cl;if(!/\\.(apk|apks|xapk|apkm)$/i.test(name)||us>80*1024*1024)continue;const ln=u16(lo+26),lx=u16(lo+28),raw=buf.slice(lo+30+ln+lx,lo+30+ln+lx+cs);let bytes=raw;if(method===8)bytes=new Uint8Array(await new Response(new Blob([raw]).stream().pipeThrough(new DecompressionStream("deflate-raw"))).arrayBuffer());if(method===0)bytes=raw;if(bytes.length)out.push({name:name.split("/").pop()||"application.apk",bytes})}return out} }return []}
