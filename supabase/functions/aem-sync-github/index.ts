  ...
  for(const repo of repos){try{
   ...
   for(const run of (runs.workflow_runs||[])){
    ...
    for(const wa of (ar.artifacts||[])){
     ...
     for(const ent of entries){const path=`${repo.full_name}/actions/${wa.id}/${ent.name}`,up=await sb.storage.from("aem-artifacts").upload(path,ent.bytes,{contentType:"application/vnd.android.package-archive",upsert:true});if(up.error)throw up.error;const sha=await hash(ent.bytes);const existingArtifact=(await sb.from("artifacts").select("id,sha256,signing_status,signing_certificate_sha256").eq("release_id",rr.data.id).eq("filename",ent.name).maybeSingle()).data;const needsSigning=kind(ent.name)==="apk"&&(!existingArtifact||existingArtifact.sha256!==sha||existingArtifact.signing_status!=="ready");const payload={release_id:rr.data.id,platform:"android",kind:kind(ent.name),filename:ent.name,download_url:`${Deno.env.get("SUPABASE_URL")}/storage/v1/object/public/aem-artifacts/${path}`,size_bytes:ent.bytes.byteLength,sha256:sha,package_identity:ent.packageName||a.package_identity||null,version_code:ent.versionCode??null,signing_status:needsSigning?"pending":"ready",signing_certificate_sha256:needsSigning?null:(existingArtifact?.signing_certificate_sha256||null)};const ins=existingArtifact?await sb.from("artifacts").update(payload).eq("id",existingArtifact.id):await sb.from("artifacts").insert(payload);if(!ins.error){artifacts++;made++;if(ent.packageName&&!a.package_identity){a.package_identity=ent.packageName;await sb.from("applications").update({package_identity:ent.packageName,updated_at:new Date().toISOString()}).eq("id",a.id)}}}
