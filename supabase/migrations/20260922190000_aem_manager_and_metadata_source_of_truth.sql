-- AEM Manager and application metadata source-of-truth additions.
-- Additive only: existing catalog/release/build/download tables remain intact.
...
insert into public.manager_state(id,status)
values ('default','idle')
...
create policy "public read manager state"
  on public.manager_state
  for select
